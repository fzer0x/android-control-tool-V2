...
class JadxInstaller(QObject):
    ...
    def run(self):
        try:
            ...
            tools_dir = os.path.join(os.getcwd(), "tools", "jadx")
    ...
    def show_method_code(self):
        ...
        if meth.get_instructions():
            for ins in meth.get_instructions():
                offset = 0
                if hasattr(ins, 'get_idx'):
                    offset = ins.get_idx()
                elif hasattr(ins, 'get_offset'):
                    offset = ins.get_offset()
                elif hasattr(ins, 'get_address'):
                    offset = ins.get_address()
